(function(OSC, $, undefined) {
    ...
    OSC.updateYOffset = function(ui,index) {
        ...
        if ( id == 'ch1_offset_arrow' || id == "ch2_offset_arrow" || id == "ch3_offset_arrow" || id == "ch4_offset_arrow") {
            $('#info_box').html('IN' + ch_n + ' ' + RP_I18N.t('osc.center_offset_info') + ' ' + OSC.convertVoltage(new_value));
            ...
            $('#info_box').html('OUT' + ch_n + ' ' + RP_I18N.t('osc.center_offset_info') + ' ' + OSC.convertVoltage(new_value));
            if ($('#out'+ch_n+'_dialog').is(':visible')){
                OSC.setOutOffsetPlotCh(ch_n)
            }
        } else if (ui.helper[0].id == 'math_offset_arrow') {
            $('#info_box').html(RP_I18N.t('osc.math_label') + ' ' + RP_I18N.t('osc.center_offset_info') + ' ' + OSC.convertVoltage(new_value));
